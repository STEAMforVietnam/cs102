...
class GameState:
    ...
    def update_state(self, state: GameStateType) -> None:
        ...
        if self.state == GameStateType.WON:
            self.status_text = FREESANSBOLD_48.render("YOU WON!! Press Enter to Retry", True, RED)
        elif self.state == GameStateType.LOST:
            self.status_text = FREESANSBOLD_48.render("YOU LOST!! Press Enter to Retry", True, RED)
