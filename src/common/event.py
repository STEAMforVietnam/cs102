...
class GameEvent:
    ...
    * Please note: this line gets and ERASES the Event queue, so we only run it exactly once
    ...
    def __init__(self, init_arg: Union[Event, EventType, int]):
        ...
        """
        if isinstance(init_arg, pygame.event.EventType):
            self.event = init_arg
        elif isinstance(init_arg, EventType):
            self.event = Event(init_arg.value)
        elif isinstance(init_arg, int):
            self.event = Event(init_arg)
