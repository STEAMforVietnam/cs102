  ...
  def _load_sprites(self, sprites_dir, scale):
    ...
    for action_enum in ActionType:
      ...
      for img_name in os.listdir(f'{sprites_dir}/{action_type}'):
        img = pygame.image.load(os.path.join(sprites_dir, action_type, img_name))
        action_sprites.append(pygame.transform.scale(img, (int(img.get_width() * scale), int(img.get_height() * scale))))
