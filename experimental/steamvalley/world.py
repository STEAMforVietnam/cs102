...
class World:
    ...
    def get_npcs(self) -> List[BaseSprite]:
        return [tile for tile in self.tiles.values() if tile.object_type in NPC_TILE_TYPES]

    def get_npc_ids(self) -> List[int]:
        return [tile.id for tile in self.tiles.values() if tile.object_type in NPC_TILE_TYPES]
